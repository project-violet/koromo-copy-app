...
namespace Koromo_Copy.Framework.Component.Hitomi
{
    public class HitomiDataSearch
    {
        ...
        public static List<HitomiIndexMetadata> GetSubsetOf(int start, int count)
        {
            ...
            {
                string lang = "n/a";
                if (v.Language >= 0) lang = HitomiData.Instance.index.Languages[v.Language];
        ...
        private static List<HitomiIndexMetadata> search_internal(HitomiDataQuery query, int starts, int ends)
        {
            ...
            for (int i = starts; i < ends; i++)
            {
                ...
                if (query.Common.Count > 0)
                {
                    ...
                    if (v.Tags != null) IntersectCountSplit(v.Tags.Select(x => HitomiData.Instance.index.Tags[x]).ToArray(), query.Common, ref check);
                    if (v.Artists != null) IntersectCountSplit(v.Artists.Select(x => HitomiData.Instance.index.Artists[x]).ToArray(), query.Common, ref check);
                    if (v.Groups != null) IntersectCountSplit(v.Groups.Select(x => HitomiData.Instance.index.Groups[x]).ToArray(), query.Common, ref check);
                    if (v.Parodies != null) IntersectCountSplit(v.Parodies.Select(x => HitomiData.Instance.index.Series[x]).ToArray(), query.Common, ref check);
                    if (v.Characters != null) IntersectCountSplit(v.Characters.Select(x => HitomiData.Instance.index.Characters[x]).ToArray(), query.Common, ref check);
                }
                bool connect = false;
                if (check.Length == 0) { check = new bool[1]; check[0] = true; }
                if (check[0] && v.Artists != null && query.Artists != null) { check[0] = IsIntersect(v.Artists.Select(x => HitomiData.Instance.index.Artists[x]).ToArray(), query.Artists); connect = true; } else if (query.Artists != null) check[0] = false;
                if (check[0] && v.Tags != null && query.TagInclude != null) { check[0] = IsIntersect(v.Tags.Select(x => HitomiData.Instance.index.Tags[x]).ToArray(), query.TagInclude); connect = true; } else if (query.TagInclude != null) check[0] = false;
                if (check[0] && v.Groups != null && query.Groups != null) { check[0] = IsIntersect(v.Groups.Select(x => HitomiData.Instance.index.Groups[x]).ToArray(), query.Groups); connect = true; } else if (query.Groups != null) check[0] = false;
                if (check[0] && v.Parodies != null && query.Series != null) { check[0] = IsIntersect(v.Parodies.Select(x => HitomiData.Instance.index.Series[x]).ToArray(), query.Series); connect = true; } else if (query.Series != null) check[0] = false;
                if (check[0] && v.Characters != null && query.Characters != null) { check[0] = IsIntersect(v.Characters.Select(x => HitomiData.Instance.index.Characters[x]).ToArray(), query.Characters); connect = true; } else if (query.Characters != null) check[0] = false;
                if (check[0] && v.Type != null && query.Type != null) { check[0] = query.Type.Any(x => v.Type >= 0 && x == HitomiData.Instance.index.Types[v.Type].Replace(' ', '_')); connect = true; } else if (query.Type != null) check[0] = false;
